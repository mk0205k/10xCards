...
import { postCodeReviewTool, postPlanReviewTool } from "../tools/post-pr-comment.js";
 ...
 * `readImplReviewCriteria`, `postCodeReview`, and `postPlanReview`. Unlike
 * `reviewCode` (the pure scorer kept for evals), this drops `Output.object` —
 * the structured review lives in the model's tool calls, and the side effect
 * (posting) happens inside the loop. The overall verdict is recovered afterward
 * from the `postCodeReview` call.
 *
...
export function createReviewRunnerAgent(opts: RunReviewOptions = {}) {
  ...
  trace(`agent created · model=${modelId} · tools=[readPlan, readImplReviewCriteria, postCodeReview, postPlanReview]`);
  ...
  return new ToolLoopAgent({
    ...
    tools: {
      ...
      postCodeReview: postCodeReviewTool,
      postPlanReview: postPlanReviewTool,
    },
 ...
 * `postCodeReview` call's review.verdict; if no code comment was posted (error
 * path), default to `fail` so a broken run never silently passes. Both
 * `postCodeReview` and `postPlanReview` count toward `postedComments`.
 */
export function recoverOutcome(steps: StepLike[]): RunReviewResult {
  const postCalls = steps
    .flatMap((step) => step.toolCalls ?? [])
    .filter((call) => call.toolName === "postCodeReview" || call.toolName === "postPlanReview");

  let verdict: "pass" | "fail" = "fail";
  for (const call of postCalls) {
    if (call.toolName === "postCodeReview") {
      const input = call.input as { review?: { verdict?: unknown } } | null;
