...
import { postCodeReviewTool, postPlanReviewTool } from "./post-pr-comment.js";
...
function runCode(payload: unknown): Promise<PostResult> {
  const execute = postCodeReviewTool.execute;
  if (!execute) throw new Error("postCodeReviewTool.execute is undefined");
  return execute(payload as never, { toolCallId: "t", messages: [] }) as Promise<PostResult>;
}

function runPlan(payload: unknown): Promise<PostResult> {
  const execute = postPlanReviewTool.execute;
  if (!execute) throw new Error("postPlanReviewTool.execute is undefined");
  return execute(payload as never, { toolCallId: "t", messages: [] }) as Promise<PostResult>;
}

describe("postCodeReviewTool", () => {
  it("posts via gh in GitHub env, passing --body-file", async () => {
    process.env.GITHUB_ACTIONS = "true";
    process.env.PR_NUMBER = "42";

    const result = await runCode({ review: codeReview });
...
describe("postPlanReviewTool", () => {
  it("renders the plan body and returns kind: 'plan'", async () => {
    const writes: string[] = [];
    vi.spyOn(process.stderr, "write").mockImplementation((chunk: string | Uint8Array): boolean => {
      writes.push(String(chunk));
      return true;
    ...
    const result = await runPlan({ review: planReview });
