...
**Contract**: `POST` handler, `export const prerender = false`. Sekwencja: (1) `user = context.locals.user; if (!user) return jsonResponse(401,...)`, (2) `supabase = createClient(context.request.headers, context.cookies)`; if null → redirect `/account?error=<encoded>` (303) z Polish message, (3) `const { error } = await supabase.rpc('enqueue_hard_delete', { p_user_id: user.id })`; error → redirect `/account?error=<encoded>` (303) z Polish message + log strukturowany, (4) `const { error: sErr } = await supabase.auth.signOut({ scope: 'global' })` — errors log-only (best effort, RLS EXISTS gate i tak blokuje wszystko po soft-delete), (5) `return context.redirect('/', 303)`.
